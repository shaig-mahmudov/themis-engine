...
**Project Name:** Pathfinder 1e Rules Engine
...
The Pathfinder 1e Rules Engine is a comprehensive backend system designed to programmatically resolve the complex mechanics of the Pathfinder First Edition tabletop role-playing game. The engine automates all mathematical and systemic calculations, including base statistics, hit points, spell slots, condition effects, and intricate item bonus stacking. By abstracting the extensive rule set into a highly cohesive domain model, this project provides a reliable, scalable foundation for character management, encounter simulation, and potential digital tabletop integrations.

### 2. Technology Stack

The infrastructure prioritizes performance, strict domain isolation, and maintainability to handle the heavily relational nature of tabletop mechanics.

| Component | Technology | Rationale |
| --- | --- | --- |
| **Language** | Java 25 | Provides mature pattern matching and record patterns to parse complex rules and conditions without massive branching logic. |
| **Framework** | Spring Boot 4.0 | Delivers robust dependency injection, REST/WebSocket capabilities, and aligns perfectly with Java 25 Long-Term Support (LTS). |
...
The system utilizes a Hexagonal Architecture (Ports and Adapters) to completely isolate the core Pathfinder rules from web controllers and database constraints. The core logic is structured using Domain-Driven Design (DDD) principles.
...
* **RuleEngine:** A domain service responsible for resolving complex, multi-entity interactions like Action Economy and Attacks of Opportunity.
